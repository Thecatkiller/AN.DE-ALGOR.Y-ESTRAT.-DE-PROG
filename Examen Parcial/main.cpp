...
enum Categoria { Fajas = 1, Correas = 2, Rodamientos = 3 };
...
int menorCantidad(Articulo arr[], int n);
int mayorCantidad(Articulo arr[], int inicio, int fin);
...
int main(int argc, char** argv) {
	...
	while (op != '9'){		
		...
		switch (op){			
			...
			case OPC_PREGUNTA1:{
				int menor = menorCantidad(lista,n);
				int cElementosMenores = 0;
				
				Articulo* listaMenores = crearArregloCantidad(lista,n,menor,cElementosMenores);
				
				cout << endl << endl << "Numero cantidad menor : " << menor << endl;
				cout << "Articulos menores :" << endl;
				mostrarLista(listaMenores,0, cElementosMenores);
				break;
			}
			...
			case OPC_MAYOR_ROD:{
				...
				Articulo* arr =crearArregloCategoria(lista,n,(Categoria)3,cantidad);
				int mayor = mayorCantidad(arr,0,cantidad - 1);
				
				cout << endl <<"Elementos de la categoria Rodamientos :" << endl;
				
				mostrarLista(arr,0,cantidad);
				
				cout << endl << "La mayor cantidad es :" << mayor << endl;
...
int menorCantidad(Articulo arr[], int n) 
{ 
    int min = arr[0].cantidad;
        
    for(int i = 1; i < n; i++) 
    { 
         if (arr[i].cantidad < min)      
            min = arr[i].cantidad; 
    } 
...
void ordenarSELDESC(Articulo lista[],int n){
	...
	for (int i=0;i<n-1;i++){
		idx = i;		
		for (int j=i+1;j<n;j++){
			if (lista[j].nombre.compare(lista[idx].nombre) > 0){		     
				idx = j;				
			}
		}
...
int mayorCantidad(Articulo arr[], int inicio, int fin){
	...
	int izq = mayorCantidad(lista,inicio, medio);
	int der = mayorCantidad(lista,medio + 1, fin);
...
char menu(){
	...
	cout << "[6] Mayor rodamientos" << endl;
	cout << "[7] Cantidad rodamientos" << endl;
	cout << "[9] Salir          " << endl;
